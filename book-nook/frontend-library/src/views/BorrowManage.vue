<template>
  <section class="paper-card page-section">
    <div class="toolbar-line"><div><p class="eyebrow">BORROW</p><h1>{{ $route.meta.title }}</h1></div><el-button class="primary-action" @click="visible=true">办理借书</el-button></div>
    ...
    <el-table :data="rows" v-loading="loading" border>
      ...
      <el-table-column label="操作" width="120"><template #default="{ row }"><el-button v-if="row.status !== 'RETURNED'" text @click="returnIt(row.id)">归还</el-button></template></el-table-column>
    </el-table>
...
async function borrow(){await api.borrow(form);ElMessage.success('借书成功');visible.value=false;load()}
async function returnIt(id){await api.returnBook(id);ElMessage.success('归还成功');load()}
onMounted(load)
</script>
