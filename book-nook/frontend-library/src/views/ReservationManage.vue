<template>
  <section class="paper-card page-section">
    <div class="toolbar-line"><div><p class="eyebrow">RESERVATION</p><h1>{{ $route.meta.title }}</h1></div><el-button class="primary-action" @click="visible=true">新增预约</el-button></div>
...
async function reserve(){await api.reserve(form);ElMessage.success('预约成功');visible.value=false;load()}
onMounted(load)
</script>
